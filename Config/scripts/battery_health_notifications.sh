...
if [[ $CABLE_PLUGGED == 'yes' ]]; then
    ...
    if [[ $BATTERY_PERCENTAGE -gt 80 ]]; then
        notify-send -a  "Battery reached 80%"  --urgency=critical "unplug the power cable to optimize" "battery life."
    fi

else

    if [[ $BATTERY_PERCENTAGE -lt 30 ]]; then
        notify-send -a "Battery is below 30%"  --urgency=critical "plug in the power cable to optimize" "battery life."
    fi
